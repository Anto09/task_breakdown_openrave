...
class TaskBreakdown:
	...
	def AppendValidStartAndEnd(self, displacement_vec, adjustment_quat, human_transform, goggles_transform, forehead_transform):
		...
		path_transform_change	 = self.transfer_path[self.state_change_index]
		...
		new_path_point_cp = Pose()
		new_path_point_cp.position = self.utils.AddPoint(self.utils.VecToPoint(path_transform_change[0:3,3]), displacement_vec)
		new_path_point_cp.orientation = self.utils.MultiplyQuaternion(self.utils.VecToQuat(transformations.quaternion_from_matrix(path_transform_change[0:3, 0:3])), adjustment_quat)
	...
	def TrajoptTransferPath(self):
		...
		for i in range (0, num_valid_right): 
			...
			if (len(r_sol) == 0 or len(r_end_sol) == 0):
				continue
