...
class TaskBreakdown:
	...
	valid_transfer_path_right 	= None
	valid_transfer_path_left	= None
	...
	def AppendValidStartAndEndTransform(self, displacement, human_transform, forehead_transform, two_arms = False):
		has_sol = False

		if (two_arms):
			T_target_start_r = np.copy(self.transfer_path_right[0] * displacement)
			T_target_start_l = np.copy(self.transfer_path_left[0] * displacement)
			
			T_target_change_r = np.copy(self.transfer_path_right[self.state_change_index]) * displacement
			T_target_change_l = np.copy(self.transfer_path_left[self.state_change_index]) * displacement

			T_target_end_r = np.copy(self.transfer_path_right[len(self.transfer_path_right) - 1]) * displacement
			T_target_end_l = np.copy(self.transfer_path_left[len(self.transfer_path_left) - 1]) * displacement


			new_path_point_start_r = self.utils.ModifyTransformToPoint(T_target_start)
			new_path_point_cp_r    = self.utils.ModifyTransformToPoint(T_target_change)
			new_path_point_end_r   = self.utils.ModifyTransformToPoint(T_target_end)

			new_path_point_start_l = self.utils.ModifyTransformToPoint(T_target_start)
			new_path_point_cp_l    = self.utils.ModifyTransformToPoint(T_target_change)
			new_path_point_end_l   = self.utils.ModifyTransformToPoint(T_target_end)

			r_start_sol = self.GetIKSolutions(T_target_start_r, self.r_manip)
			r_end_sol = self.GetIKSolutions(T_target_end_l, self.r_manip)

			l_start_sol = self.GetIKSolutions(T_target_start_r, self.l_manip)
			l_end_sol = self.GetIKSolutions(T_target_end_l, self.l_manip)
			if (r_start_sol != None and r_end_sol != None and len(r_start_sol) > 0 and len(r_end_sol) > 0 and
				l_start_sol != None and l_end_sol != None and len(l_start_sol) > 0 and len(l_end_sol) > 0):
				# print ('Valid start and end for two arm manipulation')
				self.valid_transfer_starts_right.append(new_path_point_start_r)
				self.valid_transfer_starts_left.append(new_path_point_start_l)

				self.valid_transfer_changepoints_right.append(new_path_point_cp_r)
				self.valid_transfer_changepoints_left.append(new_path_point_cp_l)

				self.valid_transfer_ends_right.append(new_path_point_end_r)
				self.valid_transfer_ends_left.append(new_path_point_end_l)

				for ppeTrans in self.transfer_path_right:
					T_target = np.copy(ppeTrans) * displacement
					valid_transfer_waypoints_right.append(T_target)

				for ppeTrans in self.transfer_path_left:
					T_target = np.copy(ppeTrans) * displacement
					valid_transfer_waypoints_left.append(T_target)
			...
			T_target_start  = np.copy(self.transfer_path[0]) * displacement
			T_target_change = np.copy(self.transfer_path[self.state_change_index]) * displacement
			T_target_end    = np.copy(self.transfer_path[len(self.transfer_path) - 1]) * displacement

			new_path_point_start = self.utils.ModifyTransformToPoint(T_target_start)
			new_path_point_cp 	 = self.utils.ModifyTransformToPoint(T_target_change)
			new_path_point_end 	 = self.utils.ModifyTransformToPoint(T_target_end)
			...
			if (r_start_sol != None and r_end_sol != None and len(r_start_sol) > 0 and len(r_end_sol) > 0):
				# print ('Valid start and end for right')
			...
			if (l_start_sol != None and l_end_sol != None and len(l_start_sol) > 0 and len(l_end_sol) > 0):
				# print ('Valid start and end for left')
	...
	def AppendValidStartAndEnd(self, displacement_vec, adjustment_quat, human_transform, forehead_transform):
	...
	def FindBestDisplacementTwoArms(self):
		...
		for a in range(0, 1): #angle discretization
			...
			#reorient person 
			quat_array 	= transformations.quaternion_about_axis(person_angle + angle, person_axis)
			...
			for i in range(0, len(self.gridPositions)):
				...
				diff = self.utils.SubPoint(grid_pos, path_start_pos) #geometry_msgs/Point
				displacement = transformations.quaternion_translation_matrix(quat_array, [diff.x, diff.y, diff.z])
				...
				human_transform *= displacement

				forehead_transform = np.copy(self.orig_forehead_transform)
				forehead_transform *= displacement

				transfer_path_start_r = np.copy(self.transfer_path_right[0]) * displacement
				transfer_path_start_l = np.copy(self.transfer_path_left[0]) * displacement

				#path start filtering
				midpoint_pos = (transfer_path_start_r[0:3,3] + transfer_path_start_l[0:3,3]) * 0.5
				if (midpoint_pos[0] <= self.robot.GetTransform()[0,3] or 
					np.fabs(midpoint_pos[0] - self.robot.GetTransform()[0,3]) > 0.6 or
					np.fabs(midpoint_pos[1] - self.robot.GetTransform()[1,3]) > 0.3 or
					np.linalg.norm(midpoint_pos - self.robot.GetTransform()[0:3,3]) > 1):
					# print("invalid due to location")
					continue

				self.TransformWorker(human_transform, forehead_transform)
				if (self.env.CheckCollision(self.robot)): #clear robot arms first (figure out later)
					# print("invalid due to collision")
					continue

				new_path_right = []
				new_path_left  = []
				# if (self.AppendValidStartAndEndTransform(displacement, human_transform, forehead_transform, True)):
				# 	print 'displacement\n', displacement, '\nhas a valid start and end'

				pidx = 0
				for p in range(0, len(self.transfer_path_right)-1):
					T_target_r = np.copy(self.transfer_path_right[p]) * displacement
					T_target_l = np.copy(self.transfer_path_left[p]) * displacement

					r_sols = self.GetIKSolutions(T_target_r, self.r_manip)
					l_sols = self.GetIKSolutions(T_target_l, self.l_manip)

					if (r_sols != None and len(r_sols) > 0 and l_sols != None and len(l_sols) > 0):
						r_valid_count += 1
						l_valid_count += 1
						r_traj.append(r_sols[0])
						l_traj.append(l_sols[0])
						new_path_right.append(T_target_r)
						new_path_left.append(T_target_l)
				...
				if (r_valid_count == len(self.transfer_path_right) and l_valid_count == len(self.transfer_path_left)): #add scoring here to pick right over left or vice versa
					best_displacement 				= displacement
					self.valid_transfer_path_right 	= new_path_right
					self.valid_transfer_path_left 	= new_path_left
					for n in range(0, len(new_path_right)):
						T_rot = new_path_right[0:3,0:3]
						self.DrawLocation(path_point, [0, 0, 1])
						self.DrawOrientation(T_rot, path_point)

					print ('for both arms, best displacement \n', displacement)
	...
	def FindBestDisplacement(self):
		...
		person_axis 		= np.array([0,0,1])
		person_quat_array 	= transformations.quaternion_about_axis(np.pi, person_axis)
		...
		for a in range(0, 1):
			...
			#person orientation
			quat_array 				= transformations.quaternion_about_axis(angle, person_axis) #np_array
			...
			new_person_quat_array 	= self.utils.QuatToArray(new_person_quat) #np_arrayy
			...
			for i in range(0, len(self.gridPositions)):
				...
				forehead_transform = np.copy(self.orig_forehead_transform)
				forehead_transform[0:3, 0:3] = transformations.quaternion_matrix(quat_array)[0:3,0:3]
				...
				if (self.AppendValidStartAndEnd(new_displacement_vec, adjustment_quat, human_transform, forehead_transform)):
				...
				if (r_valid_count == len(self.transfer_path)): #add scoring here to pick right over left or vice versa
					...
					for path_point in new_path:
						T_rot = path_point[0:3,0:3]

						# self.DrawLocation(path_point, [0, 0, 1])
						# self.DrawOrientation(T_rot, path_point)
						self.valid_transfer_path.append(path_point)
	...
	def GenerateTrajoptRequest(self, start_xyz_target, start_rot_target, end_xyz_target, end_rot_target, n_steps, manip_name, init_joint_target, end_joint_target, joint_vel_coeff, init_traj = None, stationary = False):
		request = {
		  "basic_info" : {
		  ...
		  "costs" : [
		  {
			...
			"params": {"coeffs" : [joint_vel_coeff]} # a list of length one is automatically expanded to a list of length n_dofs
		  },
		  {
			"type" : "collision",
			"name" :"cont_coll",
		...
		for i in range (0, num_valid_right):
			...
			if (len(r_start_sol) > 0):
				...
				for iters in range(0, 10):
					request = self.GenerateTrajoptRequest(start_xyz_target, start_rot_target, end_xyz_target, end_rot_target, n_steps, manip_name, r_start_sol[0], r_end_sol[0], 250.0, init_traj)
					...
					with self.env:	
						...
						for t in range(0,n_steps):
							...
							prob.AddWholePathCost(co.SlopeCostWithMemory, [(t,j) for j in xrange(7)], "ABS")
							idx += 1
			...
			end_xyz_target[0] -= 0.1
			end_xyz_target[1] -= 0.2 #3
			end_xyz_target[2] -= 0.2 #4
			direction = [0, 1, 0]
			angle = np.pi*0.5
			rot_mat = transformations.rotation_matrix(angle, direction)
			or_mat  = rot_mat * transformations.quaternion_matrix(end_rot_target)
			end_rot_target = transformations.quaternion_from_matrix(or_mat).tolist()

			start_t_target 	= openravepy.matrixFromPose(np.r_[start_rot_target, start_xyz_target])
			end_t_target 	= openravepy.matrixFromPose(np.r_[end_rot_target, end_xyz_target])

			self.DrawLocation(end_t_target, [1,0,0])
			...
			if (len(r_start_sol) > 0):
				...
				end_sol = self.utils.RankConfigurationsManipulability(self.env, r_end_sol, self.robot, self.r_manip, r_start_sol[0])
					
				for iters in range(0, 10):
					request = self.GenerateTrajoptRequest(start_xyz_target, start_rot_target, end_xyz_target, end_rot_target, n_steps, manip_name, r_start_sol[0], end_sol, 0.0, None)
					# request = self.ExtendTrajoptRequest(request, total_waypoints, manip_name)
					...
					with self.env:	
						...
						for t in range(0,n_steps):
							...
							# prob.AddCost(co.TaskDeviationCost, [(t,j) for j in xrange(7)], "ABS")#, "up%i"%t)
							prob.AddCost(self.costwrapper.ManipulabilityCost, [(t,j) for j in xrange(7)], "ABS")
							# prob.AddWholePathCost(co.LegibilityCost, [(t,j) for j in xrange(7)], "ABS") #changes for two arm planning
							prob.AddWholePathCost(co.LegibilityCostTask, [(t,j) for j in xrange(7)], "ABS") #changes for two arm planning

							# test
							# prob.AddWholePathCost(self.costwrapper.JointVelCost, [(t,j) for j in xrange(7)], "ABS") #changes for two arm planning
							idx += 1
	...
	#goggles functions
	def ExtendGoggles(self): #PPEBase.reset()
	...
	def CollapseGoggles(self): #PPEBase.collapse()
		if (self.back_strap_right != None):
			with self.env:
				#left
				self.robot.SetActiveDOFs([len(self.robot.GetJoints())-5])
